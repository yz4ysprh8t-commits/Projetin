...
Tunnel URL: https://67b479b4b200264a-152-255-124-131.serveousercontent.com
"""
# Saída também salva em results_fase1.txt
import io, os
_outbuf = io.StringIO()
_orig_print = print
...
# CONFIG
# ============================================================
SERVEO_URL    = "https://67b479b4b200264a-152-255-124-131.serveousercontent.com"
...
def run_api_central_tests(backend_url):
    ...
    # T04 Register token inválido
    try:
        sc, r = http_post(f"{backend_url}/api/rpa/register",
                          {"url": "https://test.serveousercontent.com", "token": "invalid"})
        log_result("T04: Register token inválido", sc == 403, f"status={sc}")
    except Exception as e: log_result("T04: Register token inválido", False, str(e))

    # T05 Register domínio inválido
    try:
        sc, r = http_post(f"{backend_url}/api/rpa/register",
                          {"url": "https://invalid-domain.com", "token": RPA_TOKEN})
        log_result("T05: Register domínio inválido", sc == 400, f"status={sc}")
    except Exception as e: log_result("T05: Register domínio inválido", False, str(e))
...
def run_rpa_p0_infra():
    print(bold("\n🤖 PARTE 2 — RPA GATEWAY"))
    print(bold("  [P0] Infraestrutura Básica"))
    print("-" * 50)
    ...
    r = rpa_get("/versao")
    log_result("R02: Versão RPA", "versao" in r, f"versao={r.get('versao')}")
...
def run_rpa_p1_tela():
    print(bold("\n  [P1] Controle de Tela e Navegação"))
    print("-" * 50)
    ...
    tela = r.get("tela", "")
    log_result("R07: Identificação de tela", tela in ("HOME","LOGIN","PIN","RECEBER","ENVIAR",""), f"tela={tela}")
...
def run_rpa_p2_seguranca():
    ...
    resp = safe_json(r.get("stdout", ""))
    log_result("R12: Rejeitar /pagar com HMAC inválido", resp.get("detail") == "Assinatura invalida", str(resp))
    ...
    # HMAC correto — só enfileirar, não esperar processar
...
def run_rpa_p3_pix():
    ...
    print(f"    ⏳ Aguardando worker processar {tx}...")
    resultado = None
...
def run_rpa_p4_auditoria():
    ...
    r = rpa_post("/auditar")
    log_result("R21: Auditoria forçada", "confirmados" in r, f"confirmados={r.get('confirmados')}")

    r = rpa_get("/health")
    log_result("R22: Worker ativo (auditoria periódica configurada)", r.get("worker_running") == True,
...
def run_rpa_p5_robustez():
    ...
    r = rpa_get("/health")
    log_result("R24b: Worker rodando após restart", r.get("worker_running") == True, str(r))

    r = rpa_get("/diagnostico")
    log_result("R25: Diagnóstico do sistema", "whoami" in r or "id" in r, f"keys={list(r.keys())[:5]}")
...
def main():
    print(bold("=" * 60))
    print(bold("🧪 BRANCA DE NEVE 1.0 — SUITE COMPLETA DE TESTES (FASE 1)"))
    print(bold("=" * 60))
    ...
    # ---- PARTE 1: API Central ----
    # Backend não está acessível diretamente daqui (é localhost:8001 no Termux)
    ...
    if backend_online:
        run_api_central_tests(f"{SERVEO_URL}")   # Aproximação: testes via tunnel
        # Nota: Os testes T01-T09 são da API Central. Se backend não estiver no tunnel,
        # marcamos como SKIP
    else:
        print(bold("\n📡 PARTE 1 — API CENTRAL"))
        print("-" * 50)
        print(warn("  ⚠️  Backend API Central (porta 8001) não está rodando no Termux."))
        print(warn("  ℹ️  O backend precisa de MongoDB e está pensado para rodar com Docker."))
        print(warn("  ➡️  Testes T01-T09 marcados como N/A neste ambiente standalone."))
